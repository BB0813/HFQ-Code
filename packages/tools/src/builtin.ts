...
function sanitizedChildEnv(extra?: Record<string, string | undefined>): NodeJS.ProcessEnv {
...
function parsePorcelainEntry(line: string): {
  xy: string;
  path: string;
  origPath?: string;
} | null {
...
async function gitStatusTool(workspaceRoot: string, input: Record<string, unknown>) {
  ...
  if (version.code !== 0) {
    throw new Error(
      `git not available: ${(version.stderr || version.stdout || "git --version failed").trim()}`,
    );
  }
...
async function gitDiffTool(workspaceRoot: string, input: Record<string, unknown>) {
  ...
  if (inside.code !== 0 || String(inside.stdout || "").trim().toLowerCase() !== "true") {
    return { isRepo: false, error: (inside.stderr || inside.stdout || "not a git repository").trim() };
  }
  ...
  if (diff.code !== 0) {
    throw new Error(`git diff failed: ${(diff.stderr || diff.stdout || `exit ${diff.code}`).trim()}`);
  }
...
function assertSafeGitObject(raw: string): string {
  const s = raw.trim() || "HEAD";
  if (s.length > 200) throw new Error("git object ref too long");
  // Allow HEAD, branch names, SHAs, tags, and object:path via separate path arg — not here.
...
async function gitShowTool(workspaceRoot: string, input: Record<string, unknown>) {
  ...
  const pathspec = input.path != null ? String(input.path).trim() : "";
  if (pathspec) {
    // Ensure path stays under workspace when resolved as a worktree path.
  ...
  const showTarget = pathspec
    ? `${object}:${pathspec.replace(/\\/g, "/")}`
    : object;
  // Never pass refs after bare `--` (git would treat them as pathspecs).
...
async function gitCommitTool(workspaceRoot: string, input: Record<string, unknown>) {
  ...
  // Soft block common foot-guns in the message body (not a full policy).
  ...
  for (const p of rawPaths) {
    ...
    if (!rel || rel === ".") continue;
    // Reject absolute / escape; keep as relative pathspec under workspace.
  ...
  if (paths.length) {
    ...
    if (add.code !== 0) {
      throw new Error(`git add failed: ${(add.stderr || add.stdout || `exit ${add.code}`).trim()}`);
    }
  }
