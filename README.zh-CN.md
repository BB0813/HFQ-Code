...
**1.1.9** — Windows 编程智能体（Phase-1+2 + Phase-3 + 1.0.x + 1.1.x + F1 + 更新 L1–L3 + **PTY 重连 / 编码闭环抛光**）：
...
- 更新检查：多源回退（ghproxy 镜像 → ungh → 直连 GitHub）；**L1 可选后台下载** + **L2 就绪一键确认安装** + **L3 可选静默 NSIS `/S`**（默认关；Portable 无 L3）
...
- **1.1.9:** PTY scrollback 重连 · Terminal 切页回放 · Changes 空提交说明 toast · 编码闭环抛光（非 Layout 大改）
- **1.1.8:** 更新 L3 — 可选 silentInstall · pending-install ·「安装并重启」(`/S`) · Portable 拒绝 L3 · 启动超时误报修复
- **1.1.7:** 更新 L1+L2 — `updatePolicy` · 后台自动下载 · 就绪 UI · 一键确认安装
- **1.1.6:** 压缩模型参与 LLM compact · Goal `*.goals.json` 冷启动 · `read_document`（文本/docx/pdf）· Settings `compactMaxChars` · Profile chip 热更新 · mermaid 流式安全 · Tasks 父子缩进
- **1.1.5:** Coding Profiles（6内置）· mermaid 渲染 · 技能渐进匹配 · 目标驱动字段 · 记忆链接 · bundled diagram skill
- **1.1.4:** abort 隔离（仅 deny 本会话树）；live list 附带 `permissionMode`/`planMode`；权限队列（多会话 FIFO）；Chat 消息复制/懒加载路由/CSS token
...
更新策略：检查 → 可选后台下载 → 就绪 → 确认安装；**L3 静默仅为 opt-in**（默认关；Portable 无 L3）。
...
| 版本 | 日期 | 摘要 |
|------|------|------|
| **[1.1.9](./docs/RELEASE-1.1.9.md)** | 2026-07-21 | PTY scrollback 重连 · Terminal 回放 · Changes 提交 toast · 编码闭环抛光 |
| **[1.1.8](./docs/RELEASE-1.1.8.md)** | 2026-07-20 | 更新 L3 可选静默 NSIS · pending-install · 安装并重启 · Portable 拒 L3 · 启动超时误报修复 |
| **[1.1.7](./docs/RELEASE-1.1.7.md)** | 2026-07-20 | 更新 L1+L2：后台自动下载 · 就绪 UI · 一键确认安装 · `updatePolicy` |
| **[1.1.6](./docs/RELEASE-1.1.6.md)** | 2026-07-20 | 压缩 LLM compact · Goal sidecar 冷启动 · `read_document` · compactMaxChars · UI 抛光（profile chip / mermaid 流式 / Tasks 缩进） |
| **[1.1.5](./docs/RELEASE-1.1.5.md)** | 2026-07-19 | Coding Profiles（6内置）· mermaid 渲染 · 渐进技能匹配 · 目标驱动字段 · 记忆链接 · bundled diagram |
| **[1.1.4](./docs/RELEASE-1.1.4.md)** | 2026-07-19 | abort 权限隔离；live list 附带访问模式；权限队列；Chat 消息复制/懒加载路由/CSS token 迁移 |
...
git push origin v1.1.4   # 触发 Release workflow
```
